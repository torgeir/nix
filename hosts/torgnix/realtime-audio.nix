...
}:
{
  ...
  # airplay timing exchange ports
  services.pipewire.raopOpenFirewall = true;
  # load pw airplay discovery module
  services.pipewire.extraConfig.pipewire = {
    "50-pipewire-to-airplay" = {
      "context.modules" = [
        {
  ...
  # RAOP sink volumes are persisted by WirePlumber in ~/.local/state/wireplumber/stream-properties
  # keyed by media.name (e.g. "RAOP to Bedroom"). Set all Sonos sinks at once:
  #   pw-dump | jq -r '.[] | select((.info.props["node.name"] // "") | startswith("raop_sink")) | .id' | xargs -I{} wpctl set-volume {} 0.3

  services.pipewire.extraConfig.pipewire."10-clock" = {
    "context.properties" = {
      "default.clock.quantum" = 128;
      "default.clock.allowed-rates" = [
        44100 # pw to sonos, nescessary
        48000
      ];
    };
  };
  ...
  # Remember to
  #   systemctl --user restart pipewire wireplumber
  services.pipewire.extraConfig.jack."override" = {
    # set jack quantum at runtime instead: pw-metadata -n settings 0 clock.force-quantum 128
    # node.force-quantum = 48;  # 0.001s
  ...
  services.pipewire.configPackages = [
    (pkgs.writeTextDir "share/pipewire/pipewire.conf" (
